...
async function getSearch(region: string, q: string) {
  const url = `${API}/public/region/${encodeURIComponent(region)}/search?q=${encodeURIComponent(q)}&limit=20`;
...
export default async function SearchPage({
  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px 16px" }}>
      ...
      <form action={`/${regionSlug}/search`} method="GET" style={{ display: "flex", gap: 8, marginBottom: 30, marginTop: 20 }}>
        <input
          name="q"
          defaultValue={q}
        ...
        />
        <button className={styles.btnAction} style={{ borderRadius: 12, padding: "0 24px", cursor: "pointer", border: "1px solid rgba(0,0,0,0.1)" }}>
          Найти
        </button>
      </form>

      {q.length < 2 ? (
        <div style={{ opacity: 0.7 }}>Введите минимум 2 символа для начала поиска.</div>
      ) : !data?.ok && q.length >= 2 ? (
        <div style={{ color: "crimson" }}>Ошибка загрузки результатов (Код: {data?.error}).</div>
      ) : (
        <div style={{ display: "grid", gap: 40 }}>
          
          {/* Услуги */}
          <section>
            <h2 className={styles.h2} style={{ marginBottom: 16 }}>Услуги ({services.length})</h2>
            <div className={styles.grid}>
              {services.length ? services.map((s: any) => (
                ...
                />
              )) : <div style={{ opacity: 0.6 }}>Ничего не найдено.</div>}
            </div>
          </section>

          {/* Товары */}
          <section>
            <h2 className={styles.h2} style={{ marginBottom: 16 }}>Товары ({products.length})</h2>
            <div className={styles.grid}>
          ...
          {/* Компании */}
          <section>
            <h2 className={styles.h2} style={{ marginBottom: 16 }}>Компании ({companies.length})</h2>
            <div className={styles.grid}>
